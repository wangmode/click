...
//            }
    ...
    public function getUrl($action, $vars,$listOrder =10000)
    {
        ...
        if($listOrder ==10000){
            ...
            $url = $this->where('full_url', $fullUrl)->where('list_order',$listOrder)->value('url');
        }
    ...
    {
        $fullUrl   = $this->buildFullUrl($action, $vars);
//        $findRoute = $this->where(['full_url' => $fullUrl])->find();//wjc
        if( $listOrder == 10000){
            $findRoute = $this->where(['full_url' => $fullUrl])->find();
        }else{
            $findRoute = $this->where(['full_url' => $fullUrl])->where(['list_order'=>$listOrder])->find();
        }
    ...
    public function deleteRoute($action, $vars,$listOrder = 10000)
    {
        $fullUrl = $this->buildFullUrl($action, $vars);
        if( true|$listOrder == 10000){
            $this->where(['full_url' => $fullUrl])->delete();
        }else{
            $this->where('full_url', $fullUrl)->where('list_order',$listOrder)->delete();
        }
        return true;
    }
