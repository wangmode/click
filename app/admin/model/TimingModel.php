...
class TimingModel extends Model
{
    ...
    protected $connection = [
        ...
        'type'        => 'mysql',
        // 服务器地址
        'hostname'    => 'youzhantong.mysql.rds.aliyuncs.com',
        // 数据库名
        'database'    => 'yzt-news',
        // 数据库用户名
    ...
    function news_auto_queue(){
        ...
        foreach ($user_list as $k=>$v){
            ...
            if(isset($news)){
                ...
                echo '数据处理';
                echo  date("H:i:s").'<br>';
                $result = $two_db->name('tem_news')->insertAll($news);
                echo '数据添加完成'.'<br>';
    ...
    //熊掌号审核结果查询
    function inquireReview($where=['status'=>1],$type=1){
        $site_info = cmf_get_option('site_info');
        $user = Db::name('xzh_user')->where($where)->limit($site_info['cx_each_count'])->column('id,name,third_id,remain_count');
        if($user){
            //tp信息
            ...
            foreach ($user as $k=>$v){
                ...
                if(isset($info_re['error_code'])){
                    ...
                    echo '查询失败，错误代码'.$info_re['error_code'].'，'.$info_re['error_msg'];
                }else{
                    if(isset($info_re['office_info']['auth_status']) && $info_re['office_info']['auth_status']==2){
                        //审核通过，更新状态
                        $data['remain_count'] = $v['remain_count']-1;
                        $data['status'] = 2;
                        Db::name('xzh_user')->where('id',$v['id'])->update($data);
                        if($type==2){
                        ...
                        Db::name('xzh_user')->where('id',$v['id'])->update($data);
                        if($type==2){
                            return array('status'=>1,'msg'=>'审核失败账号,状态：'.$status[$info_re['office_info']['auth_status']].$reason);
                        }
                        echo '账号状态：'.$status[$info_re['office_info']['auth_status']].$reason;exit;
                    }
                }
            }
        }else{
            if($type==2){
                return array('status'=>0,'msg'=>'没有审核中的熊掌号');
            }
            echo '没有审核中的熊掌号！';exit;
        }
    }
