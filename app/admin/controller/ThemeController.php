...
class ThemeController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $data = httpRequest(config('api_url').'/admin/api/get_tem_list.html?class='.$class,'GET');
        $remote_data = json_decode(trim($data,chr(239).chr(187).chr(191)),true);
        $this->assign('template', $remote_data['template']);
        $path = 'themes/';
        //本地模板列表
        $local_template =  get_tem($path);
        $this->assign('local_template', $local_template);
    ...
    public function active()
    {
        ...
        if ($theme == config('cmf_default_theme')) {
            return json(['status'=>0,'msg'=>'模板已启用']);
        }
/*        $themeModel = new ThemeModel();
        $themeCount = $themeModel->where('theme', $theme)->count();
        ...
        if ($result === false) {
            return json(['status'=>0,'msg'=>'配置写入失败']);
        }
        session('cmf_default_theme', $theme);
        return json(['status'=>1,'msg'=>'模板启用成功']);
    }
    ...
    public function themeDownload(){
        $zip = $this->request->param('position');
        $theme = $this->request->param('theme');
        $api_url = config('api_url');
        $url = $api_url.'/themes/'.$zip;
        $path =  'themes/'.$theme;
        if(is_dir($path)){
            return json(['status'=>0,'msg'=>'该模板已存在！']);
        }
        $themeModel = new ThemeModel();
        $result = $themeModel->downTheme($theme,$url,$path);
        if($result===false){
            return json(['status'=>0,'msg'=>'模板下载失败！']);
        }
        return json(['status'=>1,'msg'=>'下载成功！']);
    }
