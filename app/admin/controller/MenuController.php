...
class MenuController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        foreach ($result as $key => $value) {
            ...
            $add_url = url("Menu/add", ["parent_id" => $value['id'], "menu_id" => $this->request->param("menu_id")]);
            $edit_url = url("Menu/edit", ["id" => $value['id'], "menu_id" => $this->request->param("menu_id")]);
            $delete_url = url("Menu/delete", ["id" => $value['id'], "menu_id" => $this->request->param("menu_id")]);

            $result[$key]['str_manage']     = '<button type="button" class="layui-btn layui-btn-sm" onclick="window_open(\'添加内容\',\''.$add_url.'\',\'600\',\'600\')">添加</button>  
                <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" onclick="window_open(\'编辑内容\',\''.$edit_url.'\',\'600\',\'600\')">编辑</button> 
                <button type="button" class="layui-btn layui-btn-danger layui-btn-sm" onclick="menuDelete('.$value['id'].')">删除</button> ';
            $result[$key]['status']         = $value['status'] ? '<button class="layui-btn layui-btn-normal layui-btn-sm" type="button">显示</button>' : '<button class="layui-btn layui-btn-primary layui-btn-sm" type="button">隐藏</button>';
            if (APP_DEBUG) {
                $result[$key]['app'] = $value['app'] . "/" . $value['controller'] . "/" . $value['action'];
            }
        }
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                $param        = $this->request->param("param")?$this->request->param("param"):'';
    ...
    public function delete()
    {
        ...
        if ($count > 0) {
            return json(['status'=>0,'msg'=>'该菜单下还有子菜单，无法删除！']);
        }
